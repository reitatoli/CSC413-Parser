...
public class Parser {
    ...
    boolean startingStatement() {
        if (isNextTok(Tokens.If) || isNextTok(Tokens.While) || isNextTok(Tokens.Return)
                || isNextTok(Tokens.LeftBrace) || isNextTok(Tokens.Identifier) || isNextTok(Tokens.Repeat)) {
            return true;
        }
        return false;
    }
    ...
    public AST rStatement() throws SyntaxError {
        ...
        if (isNextTok(Tokens.Repeat)) {//Repeat is a do while loop, so inlcude expect tokens.While
            scan();
            t = new RepeatTree();
            t.addKid(rBlock());
            expect(Tokens.While);
